...
function generateItemNameAndType() {
  ...
  const allItems = [...Food, ...Toys, ...Charms, ...Treats, ...Potion, ...Costume, ...GroomingSupplies];
...
function generateItemRarity() {
  ...
  return { item_rarity_id: rarities[0].id, item_rarity: rarities[0].itemRarity };
}
...
export const addDropItemToInventory = async (req: Request, res: Response) => {
  try {
    ...
    const insertQuery = 'INSERT INTO items (name, type, rarity, ownerId) VALUES ($1, $2, $3, $4)';
