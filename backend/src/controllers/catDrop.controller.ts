...
export const dropRandomCat = (req: Request, res: Response) => {
...
export const adoptCat = (req: Request, res: Response) => {
  const client = pool.connect();

  const { name, type, level, user_id } = req.body;

  if (!name || !type || !level || !user_id) {
    return res.status(400).json({ message: "Something is missing in details" });
  }
  ...
  client
    .then(async (client) => {
      try {
        ...
        const getCurrentPlayerXPQuery =
          "SELECT xp FROM players WHERE user_id = $1";
        ...
        const updatePlayerXPQuery =
          "UPDATE players SET xp = $1 WHERE user_id = $2";
